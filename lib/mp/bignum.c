...
#ifdef K_AND_R
_TYPE( void )
bufToBig_be(buf, len, big)
  unsigned char *buf;
  int len;
  BigInt big;
#else
_TYPE( void ) bufToBig_be(unsigned char *buf,
		       int len,
		       BigInt big)
#endif
{
	...
	while (i > 1) {
		cp -= sizeof(Ulong);
		m = ((Ulong)cp[0]<<24)|((Ulong)cp[1]<<16)|
			((Ulong)cp[2]<<8)|((Ulong)cp[3]);
		*bp++ = m;
		i--;
	};
	m = 0;
...
#ifdef K_AND_R
_TYPE( void )
bigToBuf_be(big, bufsize, buf)
  BigInt big;
  int bufsize;
  unsigned char *buf;
#else
_TYPE( void ) bigToBuf_be(BigInt big,
		       int bufsize,
		       unsigned char *buf)
#endif
{
